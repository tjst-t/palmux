...
- **Frontend**: Svelte 5 + xterm.js（Vite でバンドル）
  ...
  - Vanilla JS 連携: Adapter パターン（`*Adapter.js` クラスが Svelte コンポーネントをラップ）
...
- **Adapter パターン**: Svelte コンポーネントを Vanilla JS から利用する場合は `*Adapter.js` クラスを使う
  - `mount()` / `unmount()` で Svelte コンポーネントのライフサイクルを管理
  - Adapter クラスはコンポーネントの `export function` にデリゲートする
- **ディレクトリ構成**:
  - `frontend/src/lib/` — Svelte コンポーネント（`.svelte`）と Adapter（`.js`）
  - `frontend/src/stores/` — Svelte stores（`.svelte.js`）
  - `frontend/js/` — 旧 Vanilla JS（Svelte コンポーネントが内部でインポート、段階的に移行中）
