...
| 用語 | 説明 |
|---|---|
| **ヘッダー** | 画面最上部のバー。ハンバーガーアイコン・セッション:ウィンドウ表示・タブ・接続状態ドロップダウンを含む |
| **ハンバーガーアイコン** (`☰`) | ヘッダー左端のアイコン。タップで**ドロワー**を開く |
| **セッション:ウィンドウ表示** | ヘッダー中央の `main:0` 形式のテキスト。現在接続中のセッション名とウィンドウインデックスを示す |
| **タブ** | ヘッダー右側の `[T][F][G]` ボタン群。Terminal / Files / Git の各ビューを切り替える（スプリットモード時のみ表示） |
...
| **ウィンドウリスト** | セッション展開時に表示されるウィンドウの一覧 |
...
| 用語 | 説明 |
|---|---|
| **ターミナルビュー** | xterm.js ターミナル + IME バー + ツールバーを表示するモード（デフォルト） |
| **ファイルブラウザビュー** | ファイルブラウザを表示するモード。ヘッダーの `[F]` タブで切り替え |
| **Git ブラウザビュー** | Git ブラウザを表示するモード。ヘッダーの `[G]` タブで切り替え |
