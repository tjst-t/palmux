...
| **ファイルプレビュー** | ファイルタップ時に表示されるコンテンツビュー。Markdown・コード（シンタックスハイライト）・画像・HTML・drawio ダイアグラムに対応 |
| **行ジャンプ** | ファイルプレビューで特定の行にスクロールしてハイライト表示する機能。`scrollToLine()` メソッドで実行 |
| **マッチハイライト** | 全文検索結果から開いたファイルの該当行を黄色で一時的にハイライトする機能。3秒後にフェードアウト |
| **プロジェクトディレクトリ** | セッションに紐づくルートディレクトリ。`ghq` リポジトリを優先検出し、なければカレントパス |
| **全文検索モード** | ファイルブラウザの検索モード切り替えで有効になるモード。ファイル内容を横断的にキーワード検索できる |
| **検索モード切替ボタン** | ファイル名検索（File）と全文検索（Grep）を切り替えるトグルボタン |
| **grep フィルターバー** | 全文検索モード時に表示されるオプションバー。glob フィルタ・大文字小文字区別（Aa）・正規表現（.*）のトグルを含む |
| **grep 検索結果** | ファイルパスごとにグルーピングされた全文検索結果。各行にマッチ箇所がハイライト表示される |
| **drawio プレビュー** | `.drawio` / `.dio` ファイルを diagrams.net の embed API を使って表示する viewer モード。iframe 内でダイアグラムをグラフィカルに閲覧できる |
| **drawio 編集モード** | drawio プレビューの Edit ボタンから遷移する editor モード。diagrams.net のグラフィカルエディタでダイアグラムを編集・保存できる。Exit で viewer に戻る |
...
| **マイクボタン** | IME バー内の入力フィールドと送信ボタンの間に配置される音声入力トグルボタン。Web Speech API 非対応ブラウザでは非表示 |
| **音声入力** | Web Speech API（SpeechRecognition）を使った音声認識機能。認識結果は IME 入力フィールドに挿入される |
| **中間結果表示** | 音声認識中の未確定テキストを IME バーの上に表示するプレビュー領域（`voice-interim-text`） |
...
| **Searcher** | 全文検索エンジンのインターフェース（`internal/grep`）。`Search()` と `Name()` メソッドを持つ |
| **RipgrepSearcher** | `rg --json` を使った最優先の全文検索エンジン。高速で .gitignore 尊重 |
| **GrepSearcher** | `grep -rn` を使ったフォールバック検索エンジン。ripgrep 未インストール時に使用 |
| **BuiltinSearcher** | 純 Go 実装のフォールバック検索エンジン。外部コマンド不要。`filepath.WalkDir` + `bufio.Scanner` |
| **NewSearcher** | `exec.LookPath` で rg → grep → builtin の優先順で最適エンジンを自動選択する関数 |
...
| 用語 | 説明 |
|---|---|
| **Portman ボタン** | ヘッダーの外部リンクアイコンボタン。セッションのプロジェクトに portman リースがある場合に表示される |
| **Portman リース** | portman が管理するポート公開情報。name・URL・status 等を含む |
| **Portman Runner** | portman コマンドの実行を抽象化する Go インターフェース（`internal/portman`）。テスト時にモック注入可能 |
...
| 用語 | 説明 |
|---|---|
| **GitHub ボタン** | ヘッダーの GitHub アイコンボタン。セッションのプロジェクトが GitHub リポジトリの場合に表示され、クリックで Issues ページを新タブで開く |
| **ParseGitHubURL** | git remote URL（SSH/HTTPS）を GitHub の HTTPS URL に変換する関数（`internal/git/parse.go`）。GitHub 以外は空文字を返す |
...
| 用語 | 説明 |
|---|---|
| **Svelte 5** | UI コンポーネントフレームワーク。runes API（`$state`, `$props`, `$derived`, `$effect`）でリアクティビティを実現 |
| **Vite** | フロントエンドビルドツール。Svelte コンパイラと連携し、開発サーバー（HMR）とプロダクションビルドを提供 |
| **Svelte コンポーネント** | `.svelte` ファイルで定義される UI コンポーネント。`frontend/src/lib/` に配置 |
| **Svelte Store** | `.svelte.js` ファイルで定義されるリアクティブ状態管理。`frontend/src/stores/` に配置 |
| **Adapter パターン** | Svelte コンポーネントを Vanilla JS から利用するための `*Adapter.js` クラス。`mount()` / `unmount()` でライフサイクルを管理し、コンポーネントの `export function` にデリゲート |
| **Bridge** | Vanilla JS から Svelte Store にアクセスするためのモジュール（`frontend/src/lib/bridge.js`） |
| **Svelte Action** | `use:action` ディレクティブで DOM 要素に命令的処理をアタッチするパターン。xterm.js やタッチハンドラで使用 |
