...
func TestManager_NewWindow(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "正常系: claude --continue コマンド付きウィンドウを作成",
			session: "main",
			winName: "",
			...
			wantErr:  false,
			wantArgs: []string{"new-window", "-t", "main", "-P", "-F", "#{window_index}\t#{window_name}\t#{window_active}", "claude --continue"},
		},
		{
			name:    "正常系: claude --model opus コマンド付きウィンドウを作成",
			session: "main",
			winName: "",
			...
			wantErr:  false,
			wantArgs: []string{"new-window", "-t", "main", "-P", "-F", "#{window_index}\t#{window_name}\t#{window_active}", "claude --model opus"},
		},
		{
			name:    "正常系: claude --continue --model sonnet コマンド付きウィンドウを作成",
			session: "dev",
			winName: "",
			...
			wantErr:  false,
			wantArgs: []string{"new-window", "-t", "dev", "-P", "-F", "#{window_index}\t#{window_name}\t#{window_active}", "claude --continue --model sonnet"},
		},
		{
			name:     "異常系: Executorがエラーを返す",
			session:  "nonexistent",
